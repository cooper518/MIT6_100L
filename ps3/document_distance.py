...
def get_frequencies(input_iterable) -> dict[str, int]:
    """
    Args:
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """
    common_words = set(list(freq_dict1.keys())+list(freq_dict2.keys()))
    # build list of frequency differences
    freq_diffs = []
    for k in common_words:
        a = freq_dict1[k] if k in freq_dict1.keys() else 0
        b = freq_dict2[k] if k in freq_dict2.keys() else 0
        freq_diffs.append(abs(a-b))
    # build list of frequency totals
    freq_totals = []
    for k in common_words:
        a = freq_dict1[k] if k in freq_dict1.keys() else 0
        b = freq_dict2[k] if k in freq_dict2.keys() else 0
        freq_totals.append(a+b)
    return round(1 - (sum(freq_diffs)/sum(freq_totals)), 2)
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for k in common_words:
        for k in common_words:
            ...
            b = freq_dict2[k] if k in freq_dict2.keys() else 0
            freq_dict[k] = a+b
    # sort list of words by frequency
    sorted_words = sorted(freq_dict, key=lambda k: freq_dict[k], reverse=True)
    # get alphabetical list of those words that have the same frequecy as the first item in the frequency list
    return sorted(list(filter(lambda x: freq_dict[x] == freq_dict[sorted_words[0]], sorted_words)))
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    """
    words = load_file(file_path).split()
    word_freq = get_frequencies(words)
    file_len = len(words)
    word_tfs = {}
    for w in word_freq:
        word_tfs[w] = word_freq[w] / file_len
    return word_tfs
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    """
    files = [load_file(fp).split() for fp in file_paths]
    num_files = len(files)
    word_idfs = {}
    # for each word across all files, calculate how many files contain that word
    for f in files:
        for w in f:
            # only need to calc idf for each word once
            if w not in word_idfs.keys():
                count = 0
                for ff in files:
                    count += 1 if w in ff else 0
                word_idfs[w] = math.log10(num_files/count)
    return word_idfs
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    return sorted([(w, tf[w]*idf[w]) for w in tf.keys()], key=lambda x: x[1])
...
if __name__ == "__main__":
    ...
    freq_dict1, freq_dict2 = {"hello": 1, "world": 2}, {"hello": 1, "world": 5}
